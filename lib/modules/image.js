...
module.exports = {
    ...
    save: async function (options) {
        ...
        // Check if path looks like a file instead of a directory
        if (extname(path)) {
            throw new Error(
                `image.save: path should be a directory, not a file. ` +
                `Got "${path}". Use path: "/" and template: "filename.ext" instead.`
            );
        }
    ...
    crop: async function (options) {
        ...
        // Validate required parameters exist (can be number or string) before transformation
        this.parseRequired(options.width, '*', 'image.crop: width is required.');
        this.parseRequired(options.height, '*', 'image.crop: height is required.');
        this.parseRequired(options.x, '*', 'image.crop: x is required.');
        this.parseRequired(options.y, '*', 'image.crop: y is required.');

        let width = cw(this.parse(options.width), sharp.metadata);
        let height = ch(this.parse(options.height), sharp.metadata);
        if (width > sharp.metadata.width) width = sharp.metadata.width;
        if (height > sharp.metadata.height) height = sharp.metadata.height;
        let left = cx(this.parse(options.x), width, sharp.metadata);
        let top = cy(this.parse(options.y), height, sharp.metadata);
    ...
    watermark: async function (options) {
        ...
        // Validate x and y exist (can be number or string) before transformation to prevent NaN
        this.parseRequired(options.x, '*', 'image.watermark: x is required.');
        this.parseRequired(options.y, '*', 'image.watermark: y is required.');

        let image = Sharp(path);
        let metadata = await image.metadata();
        let input = await image.toBuffer();
        let left = cx(this.parse(options.x), metadata.width, sharp.metadata);
        let top = cy(this.parse(options.y), metadata.height, sharp.metadata);
    ...
    text: async function (options) {
        ...
        let svg = `
            <svg width="${width}" height="${height}">
                ...
                <text x="${x}" y="${y}" text-anchor="${anchor}" class="text">${escapeHTML(text)}</text>
            </svg>
        `;
