...
exports.rooms = function(options) {
    return this.socket ? Array.from(this.socket.rooms).filter(room => room !== this.socket.id) : [];
};
...
exports.allRooms = async function(options) {
    ...
    if (this.io) {
        ...
        if (typeof adapter.allRooms == 'function') {
            rooms = Array.from(await adapter.allRooms());
        } else if (adapter.rooms) {
            rooms = Array.from(adapter.rooms.keys());
        }

        if (typeof adapter.allSockets == 'function') {
            let allSockets = await adapter.allSockets();
            rooms = rooms.filter(room => !allSockets.has(room));
        } else if (adapter.sids) {
            rooms = rooms.filter(room => !adapter.sids.has(room));
        }
    }
