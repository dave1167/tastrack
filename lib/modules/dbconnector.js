...
module.exports = {
                ...
                const validate = (row, rule) => {
                    if (rule.operator) {
                        ...
                        switch (rule.operator) {
                            ...
                            case 'between': return b[0] <= a && a <= b[1];
                            case 'not_between': return !(b[0] <= a && a <= b[1]);
    ...
    count: async function(options, name, meta) {
    ...
    paged: async function(options, name, meta) {
        ...
        sql.offset = Number(sql.offset);
        sql.limit = Number(sql.limit);
        if (isNaN(sql.offset)) sql.offset = Number(this.parseOptional('{{ $_GET.offset }}', '*', 0));
        if (isNaN(sql.limit)) sql.limit = Number(this.parseOptional('{{ $_GET.limit }}', '*', 25));
        if (typeof sql.sort != 'string') sql.sort = this.parseOptional('{{ $_GET.sort }}', 'string', null);
        if (typeof sql.dir != 'string') sql.dir = this.parseOptional('{{ $_GET.dir }}', 'string', 'asc');
...
async function _processSubQueries(db, results, sub, meta, prefix = '') {
    ...
    for (const field in sub) {
        ...
        const key = (sql.joins && sql.joins.length) ? (sql.table.alias || sql.table.name || sql.table) + '.' + sql.key : sql.key;
        const subResults = await db.fromJSON(sql, submeta).whereIn(key, Array.from(keys));
